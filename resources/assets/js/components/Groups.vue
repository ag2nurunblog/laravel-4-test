<template>
  <div>
    <ul class="nav nav-tabs">
  			<li class="active">
          <a  href="#list" data-toggle="tab">Groups</a>
  			</li>
  			<li>
        ...
        <div class="input-group">
          <input type="text" class="form-control" v-model="createGroup.name">
    ...
    methods: {
      ...
      create() {
        if (this.createGroup.name) {
          this.$http.post(this.url, this.createGroup).then((response) => {
            if (response.data.id) {
              this.items.unshift({
                id: response.data.id,
      ...
      edit(index) {
        this.editing = index;
        this.editGroup.name = this.items[index].name;
        setTimeout(() => {
      ...
      save() {
        if (this.editGroup.name) {
          ...
          this.$http.put(this.url + '/' + id, this.editGroup).then((response) => {
            this.items[this.editing].name = this.editGroup.name;
            this.editGroup.name = '';
            this.editing = -1;
            $('.nav-tabs [href="#list"]').tab('show');
          }, () => {
